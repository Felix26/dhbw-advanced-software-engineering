...
Trips FakeTripParser::parse()
{
    ...
    auto ice = std::make_shared<TrainTrip>(1001, freiburg, mannheim, 200000, 90, time1, TransportType::Fernverkehr, "ICE 75", dbFern);
    ice->addStopover(offenburg);
    ice->addStopover(karlsruhe);
    result.push_back(ice);

    // 2. Regionalverkehr: RE7 von Offenburg nach Freiburg
    auto re7 = std::make_shared<TrainTrip>(1002, offenburg, freiburg, 63000, 45, time2, TransportType::Regionalverkehr, "RE7", dbRegio);
